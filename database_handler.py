...
def calculate_dish_total_nutrition(dish_id, visited=None):
    """Recursively calculate the total nutrition content of a dish.
    
    This function traces through all ingredients (including sub-dishes) to calculate
    the exact total nutrition content by:
    1. For ingredient items: multiply per-gram nutrition by amount
    2. For dish items: recursively calculate their nutrition and multiply by amount
    
    Args:
        dish_id: ID of the dish to calculate
        visited: set of visited dish IDs to prevent circular references
    
    Returns:
        dict: Dictionary mapping nutrient names to total amounts
    """
    if visited is None:
        visited = set()
    
    # Prevent circular references
    if dish_id in visited:
        return {}
    visited.add(dish_id)
    
    # Load data
    dishes = _load_table('dish')
    ingredients = _load_table('ingredient')
    ...
    for req in dish.get('required_ingredients', []):
        ...
        amount_g = req.get('amount_g', 0)
        
        if item_type == 'ingredient':
            # Get ingredient nutrition (per gram)
            ingredient = next((i for i in ingredients if i['id'] == item_id), None)
            if ingredient and 'nutrition' in ingredient:
                for nutrient in ingredient['nutrition']:
                    nutrient_name = nutrient.get('name')
                    per_gram = nutrient.get('amount_per_unit_mass', 0)
                    total_amount = per_gram * amount_g
                    
                    if nutrient_name in nutrition_totals:
                        nutrition_totals[nutrient_name] += total_amount
                    else:
                        nutrition_totals[nutrient_name] = total_amount
        
        elif item_type == 'dish':
            # Recursively calculate sub-dish nutrition
            sub_dish_nutrition = calculate_dish_total_nutrition(item_id, visited.copy())
            
            # The sub-dish nutrition is already in total amounts for that dish
            # We need to get the sub-dish's per-gram nutrition and multiply by our amount
            sub_dish = next((d for d in dishes if d['id'] == item_id), None)
            if sub_dish:
                sub_dish_mass = get_dish_total_mass(sub_dish)
                if sub_dish_mass > 0:
                    # Convert sub-dish total nutrition to per-gram, then multiply by our amount
                    for nutrient_name, total_amount in sub_dish_nutrition.items():
                        per_gram = total_amount / sub_dish_mass
                        our_amount = per_gram * amount_g
                        
                        if nutrient_name in nutrition_totals:
                            nutrition_totals[nutrient_name] += our_amount
                        else:
                            nutrition_totals[nutrient_name] = our_amount
