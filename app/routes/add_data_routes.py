...
def add_ingredient_route():
    """식재료(ingredient) 추가 페이지 및 처리"""
    if request.method == 'POST':
        try:
            # 폼 데이터 추출
            name_kor = request.form['name_kor']
            name_eng = request.form['name_eng']
            research_ids = [int(x.strip()) for x in request.form['research_ids'].split(',') if x.strip()]
    ...
    return render_template('add_ingredient_form.html')
...
def add_cooking_method_route():
    if request.method == 'POST':
        try:
            ...
            flash(f"오류가 발생했습니다: {e}", 'danger')
    return render_template('add_cooking_method_form.html')
...
def add_dish_route():
    if request.method == 'POST':
        try:
            ...
            req_ing_ids = [int(x.strip()) for x in request.form['required_ingredient_ids'].split(',') if x.strip()]
            req_cook_ids = [int(x.strip()) for x in request.form['required_cooking_method_ids'].split(',') if x.strip()]
            calories = float(request.form['calories'])
